...
should_interp1d = False 		# Turn on interp1d interpolation for band structures NOT VERY WELL TESTED!
output_x_axis = True		# Whether to output the x-axis (e.g. the e=0 line) or not interp1d_factor = 10		# If interp1d interpolation turned on, the sampling factor (1 is the original grid)
...
if PLOT_BANDS:
    ...
    for start,end,length,npoint,startname,endname in band_segments:
		...
		# Apply energy offset if specified to all band energies just read in
            ...
            # Now perform interp1d interpolation on band structure if requested
        if should_interp1d == True:
            xvals_smooth = np.linspace(xvals.min(),xvals.max(),interp1d_factor*len(xvals) ) # Interpolated x axis for interp1d smoothing
            new_band_energies = []
            for b in range(band_energies.shape[1]): # interp1d every band, one by one
                new_band_energies.append(interp1d(xvals, band_energies[:,b], xvals_smooth))
...
if should_interp1d:
	print()
	print("Spine interpolation has been used on the band structure, with an interpolation factor of ", interp1d_factor)
